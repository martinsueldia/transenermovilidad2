sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "TransenerMovilidad.view.defects.stations.newDefect", {
        _newDefectModel: {},
        _oTechLocModel: {},
        _oRouter: null,
        _isEdit: false,
        ...
        _onPartChange: function (oEvent) {
          ...
          try {
            var sGroupCatalog = oView
              .getModel("NewDefect")
              .getProperty("/DlCodegrp");
        ...
        _showPhotoAlert: function () {
          let oPhotoConfig = this.getView()
            .getModel("PhotoConfigurationModel")
            .getData();
        ...
        _validPhotos: function () {
          let iValidPhotos = 0;
          let iFileLimit = this.getView()
            .getModel("PhotoConfigurationModel")
            .getProperty("/FileLimit");
          let aAttachments = this.getView()
            .getModel("NewDefect")
            .getProperty("/Attachments");
          let iMaxSizePerImage = this.getView()
            .getModel("PhotoConfigurationModel")
            .getProperty("/MaxSizePerImage");
          if (aAttachments) {
            if (aAttachments.length <= iFileLimit) {
              for (let oAttachment of aAttachments) {
                let sAttachmentType = oAttachment.type
                  .split("/")[1]
                  .toUpperCase();
        ...
        _validateStates: function () {
          let oNewDefectData = this.getView()
            .getModel("NewDefect")
            .getData();
          let oModelState = this.getView()
            ...
            if (
              ...
              if (!oNewDefectData[attr]) {
                if (attr === "Longtext" && this._isEdit) {
                  this.getView()
                    .getModel("ValidationModel")
                    .setProperty("/state" + attr, sap.ui.core.ValueState.None);
                } else {
                  this.getView()
                    .getModel("ValidationModel")
                    .setProperty("/state" + attr, sap.ui.core.ValueState.Error);
                }
              } else {
                this.getView()
                  .getModel("ValidationModel")
          ...
          if (localStorage.getItem("speciality") === "ET") {
            return (
              oModelState.stateLongtext === "None" &&
              oModelState.stateBtpln === "None" &&
              oModelState.stateDlCodegrp === "None" &&
              oModelState.stateDlCode === "None"
            );
          } else {
            return (
              oModelState.stateMarkerStart === "None" &&
              oModelState.stateMarkerEnd === "None" &&
              oModelState.stateLongtext === "None" &&
              oModelState.stateBtpln === "None" &&
              oModelState.stateDlCodegrp === "None" &&
              oModelState.stateDlCode === "None"
        ...
        saveAttachmentsSQLite: function (
          aAttachments,
          fnCallbackSuccess,
          fnCallbackErr
        ) {
        ...
        _saveAttachments: function (
          oNewDefect,
          sAttachmentOfflineId,
          fnCallbackSuccess,
          fnCallbackErr
        ) {
          let aAttachments;
          ...
          if (oNewDefect.Attachments && oNewDefect.Attachments.length > 0) {
            ...
            aAttachmentsSqlite.map(
              oAttachment => (oAttachment.Qmnum = sAttachmentOfflineId)
            );
            let aAttachmentPromises = this.getAttachmentPromises(
              aAttachmentsSqlite
            );
            Promise.all(aAttachmentPromises)
              .then(() => {
                delete oNewDefect.Attachments;
                File.handleAttachmentEntries(
                  aAttachments,
                  () => {
                    this.saveAttachmentsSQLite(
                      aAttachmentsSqlite,
                      fnCallbackSuccess,
                      fnCallbackErr
                    );
        ...
        _initDefectModel: function (oDefect) {
          ...
          if (this._isEdit) {
            oDefect.NotifDate = new Date(
              oDefect.NotifDate.getTime() +
              oDefect.NotifDate.getTimezoneOffset() * 60000
            );
        ...
        setWerks: function (oEvent) {
          var sWerks = oEvent
            ...
            .getBindingContext("WorkPlaceModel")
            .getObject().Werks;
          this.getView()
            .getModel("NewDefect")
            .setProperty("/Planplant", sWerks);
        },
        ...
        _loadPhotoConfigurationModel: function () {
          ...
          this.getView().setModel(
            oPhotoConfigurationModel,
            "PhotoConfigurationModel"
          );
        },
        ...
        _onGetPartsByLocSuccess: function (aParts) {
          if (aParts.length > 0) {
            ...
            let aPartsResults = oLocation.CatalogXUbTec_nav.results ?
              oLocation.CatalogXUbTec_nav.results : [];
        ...
        _onDownloadPress: function () {
          try {
            //todo com nohay dron ir directo al success
        ...
        _initializeDefectViewModel: function () {
          ...
          oModel.setData({
            ...
            isEdit: this._isEdit,
            isNew: !this._isEdit,
            isCorrectedDefect: oDefect.Asignar ?
              oDefect.Asignar.length > 0 : false,
        ...
        _correctDefect: function () {
          ...
          oDefectModel.setProperty(
            "/Asignar",
            WorkPlaceHelper.getCurrentWorkOrder()
          );
          this._onSaveDefect();
        }
      }
